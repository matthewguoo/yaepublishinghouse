...
const timelineDatabaseId = process.env.NOTION_TIMELINE_DATABASE_ID;
const articleDatabaseId = process.env.NOTION_ARTICLE_DATABASE_ID;
...
export const getTimelineDatabase = cache(async () => {
  const response = await notion.databases.query({
    database_id: timelineDatabaseId,
  });
  return response.results;
});

export const getArticleDatabase = cache(async () => {
  const response = await notion.databases.query({
...
export const getArticlePageFromSlug = cache(async (slug) => {
  const response = await notion.databases.query({
    database_id: articleDatabaseId,
    filter: {
      property: 'Slug',
